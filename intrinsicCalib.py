...
parser.add_argument('-mode', '--SELECT_MODE', default='manual', type=str, help='Image Select Mode: auto/manual')
...
class CalibMode():
    ...
    def cameraAutoMode(self):
        cap = cv2.VideoCapture("rtsp://192.168.1.10:554/user=admin&password=&channel=2&stream=0?.sdp")
        if not cap.isOpened():
    ...
    def cameraManualMode(self):
        cap = cv2.VideoCapture("rtsp://192.168.1.10:554/user=admin&password=&channel=3&stream=0?.sdp")
        if not cap.isOpened():
            raise Exception("from {} read video failed".format(args.CAMERA_ID))
        cap = self.setCamera(cap)
        while True:
